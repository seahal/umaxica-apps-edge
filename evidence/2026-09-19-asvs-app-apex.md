# OWASP ASVS 5.0 Level 1–2 review: `app/apex`
...
- `pnpm --dir app/apex audit --prod --json`: 0 info / 0 low / 0 moderate / 0 high / 0 critical.
- `pnpm --dir app/apex run test` (via `pnpm -r run test`): PASS, 19 files / 110 tests.
- `grep` over `app/apex/src` for `dangerouslySetInnerHTML`, `innerHTML`, `eval(`,
...
| V2 Validation/business logic                         | PASS          | Host allowlist (421), `Content-Encoding` ≠ identity → 415, `bodyLimit` 64 KiB → 413, 3 s response timeout. Region redirect is a fixed allowlist (`root-redirect.ts`), no open redirect.                      |
...
Production `ratelimits` namespace ids in this unit: `2001 3001 1001 `.
