# OWASP ASVS 5.0 Level 1–2 review: `net/apex`
...
- `pnpm --dir net/apex audit --prod --json`: 0 info / 0 low / 0 moderate / 0 high / 0 critical.
- `pnpm --dir net/apex run test` (via `pnpm -r run test`): PASS, 18 files / 91 tests.
- `grep` over `net/apex/src` for `dangerouslySetInnerHTML`, `innerHTML`, `eval(`,
...
- Source diffed against its family reference (`app/apex`): differences are brand
  copy/constants only (no `root-redirect.ts` in this unit).
...
Production `ratelimits` namespace ids in this unit: `2004 3004 1004 `.
