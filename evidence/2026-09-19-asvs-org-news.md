# OWASP ASVS 5.0 Level 1–2 review: `org/news`
...
- `pnpm --dir org/news audit --prod --json`: 0 info / 0 low / 0 moderate / 0 high / 0 critical.
- `pnpm --dir org/news run test` (via `pnpm -r run test`): PASS, 35 files / 404 tests.
- `grep` over `org/news/src` for `dangerouslySetInnerHTML`, `innerHTML`, `eval(`,
