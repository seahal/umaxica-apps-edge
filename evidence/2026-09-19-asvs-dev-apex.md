# OWASP ASVS 5.0 Level 1–2 review: `dev/apex`
...
- `pnpm --dir dev/apex audit --prod --json`: 0 info / 0 low / 0 moderate / 0 high / 0 critical.
- `pnpm --dir dev/apex run test` (via `pnpm -r run test`): PASS, 18 files / 91 tests.
- `grep` over `dev/apex/src` for `dangerouslySetInnerHTML`, `innerHTML`, `eval(`,
...
Production `ratelimits` namespace ids in this unit: `2005 3005 1005 `.
