# OWASP ASVS 5.0 Level 1–2 review: `com/docs`
...
- `pnpm --dir com/docs audit --prod --json`: 0 info / 0 low / 0 moderate / 0 high / 0 critical.
- `pnpm --dir com/docs run test` (via `pnpm -r run test`): PASS, 35 files / 404 tests.
- `grep` over `com/docs/src` for `dangerouslySetInnerHTML`, `innerHTML`, `eval(`,
...
- Source diffed against its family reference (`app/docs`): differences are brand
  copy/constants only.
...
- **F1 — Medium (V2.4 anti-automation / V13 configuration), CONFIRMED.** This
  `com` unit uses the `app` brand's namespace ids (`1001`…`5001`). ADR 010
