# OWASP ASVS 5.0 Level 1–2 review: `com/news`
...
- `pnpm --dir com/news audit --prod --json`: 0 info / 0 low / 0 moderate / 0 high / 0 critical.
- `pnpm --dir com/news run test` (via `pnpm -r run test`): PASS, 35 files / 404 tests.
- `grep` over `com/news/src` for `dangerouslySetInnerHTML`, `innerHTML`, `eval(`,
...
- Source diffed against its family reference (`app/news`): differences are brand
  copy/constants only.
