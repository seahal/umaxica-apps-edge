# OWASP ASVS 5.0 Level 1–2 review: `app/help`
...
- `pnpm --dir app/help audit --prod --json`: 0 info / 0 low / 0 moderate / 0 high / 0 critical.
- `pnpm --dir app/help run test` (via `pnpm -r run test`): PASS, 35 files / 404 tests.
- `grep` over `app/help/src` for `dangerouslySetInnerHTML`, `innerHTML`, `eval(`,
