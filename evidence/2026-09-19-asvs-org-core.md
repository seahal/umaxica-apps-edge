# OWASP ASVS 5.0 Level 1–2 review: `org/core`
...
- `pnpm --dir org/core audit --prod --json`: 0 info / 0 low / 0 moderate / 0 high / 0 critical.
- `pnpm --dir org/core run test` (via `pnpm -r run test`): PASS, 34 files / 477 tests.
- `grep` over `org/core/src` for `dangerouslySetInnerHTML`, `innerHTML`, `eval(`,
...
- Source diffed against its family reference (`app/core`): differences are brand
  copy/constants only, plus the staff publishing hub (\`publishing-hub.ts\`, \`rails-staff-origin*.ts\`, \`_page.publishing.tsx\`), which renders links to a validated \`RAILS_STAFF_BASE_ORIGIN\` only.
...
Production `ratelimits` namespace ids in this unit: `5003 5103 2003 2103 3003 3103 1003 1103 `.
