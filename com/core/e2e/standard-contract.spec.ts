 ...
 * worker registers and activates, and that a real navigation falls back to
 * the offline document when the network is gone.
 */
...
test('does not replace a Rails API navigation with the offline document', async ({
  page,
  context,
}) => {
  await page.goto('/');
